...
const EditSessionModal = ({ session, onUpdate, onClose, currentUser }) => {
  const [editingSession, setEditingSession] = useState({
    ...
    notes: session?.notes || '',
    auto_generated: session?.auto_generated || 0
  });
  ...
  useEffect(() => {
    if (session) {
      let backendDate = session.date;
      
      if (backendDate && backendDate.includes('T')) {
      ...
      const inTime = parseTimeString(session.in_time);
      const outTime = parseTimeString(session.out_time);
      
      setTimeSlots({
        inTime: inTime || { hour: '09', minute: '00', period: 'AM' },
        outTime: outTime || { hour: '05', minute: '00', period: 'PM' }
      });
    }
  }, [session]);
  ...
  const validateForm = () => {
    ...
    if (outTimeStr && inTimeStr) {
      const inDateTime = new Date(`${editingSession.date} ${inTimeStr}`);
      const outDateTime = new Date(`${editingSession.date} ${outTimeStr}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onUpdate(sessionDataWithUpdater);
    } catch (error) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">
                {session?.username || 'Employee'} • {formatDisplayDate(editingSession.date)}
              </p>
            </div>
          </div>
          <button 
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 px-4 py-3 rounded-lg border-2 transition-all duration-200 font-medium ${
                      editingSession.session_type === 'work' 
                  ...
                  <button
                    ...
                    className={`flex-1 px-4 py-3 rounded-lg border-2 transition-all duration-200 font-medium ${
                      editingSession.session_type === 'break' 
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-white font-semibold text-lg">
                        {formatDisplayDate(editingSession.date)}
                      </div>
            ...
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                <div>
                  ...
                  <div className="text-blue-300">{editingSession.date}</div>
                </div>
                <div>
