...
function OverduePayments() {
  ...
  // Access control - only user_type 0-9
  if (!currentUser || (currentUser.user_type !== undefined && (currentUser.user_type < 0 || currentUser.user_type > 9))) {
    return (
      <div className="min-h-screen bg-[#0e1830] text-white flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">Access Denied</h1>
          <p>You need to be logged in as a worker (user_type 0-9) to access this page.</p>
        </div>
      </div>
    );
  }
